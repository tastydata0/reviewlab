...
def render_settings_form(
    effective: CascadingSettings,
    inherited: CascadingSettings,
    explicit: dict,
    action_url: str,
    ...
    inherited_values = {
        ...
        "view_results_after": (
            inherited.view_results_after.isoformat()[:16]
            if inherited.view_results_after
            else ""
        ),
    }
    ...
    def fld(name, label_text, input_el, exp_val, val_disp=None, is_group=False):
        ...
        if parent_name:
            ...
            toggle = Label(
                Input(
                    type="checkbox",
                    name=f"inherit_{name}",
                    checked=is_inherited,
                    onchange=js_action,
                ),
        ...
        return Div(
            Div(
                B(label_text),
                toggle,
                style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; border-bottom: 1px solid #eee; padding-bottom: 5px;",
            ),
            input_el,
    ...
    return Form(
        ...
        H3(
            "Антиплагиат",
            style="margin-top: 20px; border-bottom: 2px solid #333; padding-bottom: 5px;",
        ),
        fld(
            "plag_max_z",
            "Порог Z-score (max_z) - чем меньше, тем строже",
            Input(
                name="plag_max_z",
                value=effective.plagiarism.max_z,
                type="range",
                step="0.1",
                min="0.5",
                max="5.0",
                oninput="updateValue('plag-z-val', this.value);",
                style="width: 100%;",
            ),
            plag_explicit.get("max_z"),
            P(
                B("Текущий порог Z-score: "),
                Span(effective.plagiarism.max_z, id="plag-z-val"),
                style="margin-top: 5px; font-size: 0.9em; color: #555;",
            ),
        ),
        fld(
            "plag_use_semantic",
            "Семантический анализ (векторы)",
            Input(
                name="plag_use_semantic",
                type="checkbox",
                checked=effective.plagiarism.use_semantic,
            ),
            plag_explicit.get("use_semantic"),
        ),
        fld(
            "plag_use_lexical",
            "Лексический анализ (текст)",
            Input(
                name="plag_use_lexical",
                type="checkbox",
                checked=effective.plagiarism.use_lexical,
            ),
            plag_explicit.get("use_lexical"),
        ),
        fld(
            "plag_min_threshold",
            "Минимальное сходство для отображения",
            Input(
                name="plag_min_threshold",
                value=effective.plagiarism.min_similarity_threshold,
                type="range",
                step="0.05",
                min="0.0",
                max="1.0",
                oninput="updateValue('plag-min-val', this.value);",
                style="width: 100%;",
            ),
            plag_explicit.get("min_similarity_threshold"),
            P(
                B("Текущий минимум: "),
                Span(effective.plagiarism.min_similarity_threshold, id="plag-min-val"),
                style="margin-top: 5px; font-size: 0.9em; color: #555;",
            ),
        ),
        H3(
            "Линтер",
            style="margin-top: 30px; border-bottom: 2px solid #333; padding-bottom: 5px;",
        ),
        fld(
            "linter_timeout",
            ...
            Input(
                name="linter_timeout",
                value=effective.linter.timeout_seconds,
                type="range",
                min="5",
                max="120",
                step="5",
                oninput="updateValue('linter-timeout-val', this.value);",
                style="width: 100%;",
            ),
            lint_explicit.get("timeout_seconds"),
            P(
                B("Текущий таймаут: "),
                Span(effective.linter.timeout_seconds, id="linter-timeout-val"),
                " сек",
                style="margin-top: 5px; font-size: 0.9em; color: #555;",
            ),
        ),
        fld(
            ...
            Select(
                Option(
                    "Стандартный",
                    value="1",
                    selected=(effective.linter.linter_preset_id == 1),
                ),
                Option(
                    "Строгий",
                    value="2",
                    selected=(effective.linter.linter_preset_id == 2),
                ),
                Option(
                    "Google Style",
                    value="3",
                    selected=(effective.linter.linter_preset_id == 3),
                ),
                Option(
                    "Академический",
                    value="4",
                    selected=(effective.linter.linter_preset_id == 4),
                ),
                name="linter_preset",
            ),
        ...
        fld(
            "llm_strictness",
            "Уровень строгости ИИ (1-10)",
            Input(
                type="range",
                name="llm_strictness",
                min="1",
                max="10",
                value=effective.llm.strictness_level,
                oninput="updateStrictness(this.value);",
                style="width: 100%;",
            ),
            ...
            Div(
                P(
                    B("Текущий уровень: "),
                    Span(effective.llm.strictness_level, id="strictness-val"),
                    style="margin-top: 5px; margin-bottom: 5px; font-size: 0.9em; color: #555;",
                ),
                Div(
                    MENTOR_PROMPTS.get(effective.llm.strictness_level, ""),
                    id="strictness-desc",
                    style="font-style: italic; color: #666; min-height: 3em; background: #f9f9f9; padding: 10px; border-radius: 5px;",
                ),
            ),
        ),
        fld(
            ...
            Select(
                Option(
                    "Сократический (намеки)",
                    value="socratic",
                    selected=(effective.llm.review_mode == "socratic"),
                ),
                Option(
                    "Прямой (объяснение ошибок)",
                    value="direct",
                    selected=(effective.llm.review_mode == "direct"),
                ),
                name="llm_mode",
            ),
        ...
        fld(
            "llm_inst_1",
            "Дополнительная инструкция 1",
            Textarea(
                effective.llm.custom_instruction_1 or "",
                name="llm_inst_1",
                rows="2",
                style="width: 100%;",
                placeholder="Слот 1 (например, для всего курса)",
            ),
        ...
        fld(
            ...
            Textarea(
                effective.llm.custom_instruction_2 or "",
                name="llm_inst_2",
                rows="2",
                style="width: 100%;",
                placeholder="Слот 2 (например, для лабы)",
            ),
            llm_explicit.get("custom_instruction_2"),
        ),
        fld(
            "llm_inst_3",
            "Дополнительная инструкция 3",
            Textarea(
                effective.llm.custom_instruction_3 or "",
                name="llm_inst_3",
                rows="2",
                style="width: 100%;",
                placeholder="Слот 3 (например, для конкретной задачи)",
            ),
        ...
        fld(
            "submission_limit",
            "Лимит попыток (0 - без лимита)",
            Input(
                name="submission_limit",
                value=effective.submission_limit,
                type="range",
                min="0",
                max="20",
                step="1",
                oninput="updateValue('submission-limit-val', this.value == 0 ? 'Без лимита' : this.value);",
                style="width: 100%;",
            ),
            ...
            P(
                ...
                Span(
                    (
                        "Без лимита"
                        if effective.submission_limit == 0
                        else effective.submission_limit
                    ),
                    id="submission-limit-val",
                ),
                style="margin-top: 5px; font-size: 0.9em; color: #555;",
            ),
        ),
        fld(
            "view_results_after",
            "Скрыть результаты до (дата публикации)",
            Input(
                name="view_results_after",
                value=(
                    effective.view_results_after.isoformat()[:16]
                    if effective.view_results_after
                    else ""
                ),
                type="datetime-local",
                style="width: 100%;",
        ...
        Button(
            ...
            style="margin-top: 20px; width: 100%; font-size: 1.1em; padding: 12px;",
        ),
        method="post",
        action=action_url,
        ...
        return Titled(
            ...
            Main(
                ...
                render_settings_form(
                    ...
                    course_settings,
                    current_settings,
                    f"/courses/{course_id}/labs/{lab_id}/settings",
                    parent_name="Курса",
                ),
                _class="container",
            ),
