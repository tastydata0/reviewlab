...
class CourseService:
    ...
    async def create_course(self, name: str, teacher_id: uuid.UUID, description: Optional[str] = None) -> Course:
    ...
    async def update_course(self, course_id: uuid.UUID, name: Optional[str] = None, description: Optional[str] = None) -> Course:
    ...
    async def get_course_users(self, course_id: uuid.UUID) -> List[User]:
        subquery = select(CourseUserLink.user_id).where(CourseUserLink.course_id == course_id)
        statement = select(User).where(User.id.in_(subquery))
        result = await self.session.execute(statement)
        users = result.scalars().all()
        print(f"DEBUG: get_course_users returned {len(users)} users. First item type: {type(users[0]) if users else 'N/A'}")
        return users
