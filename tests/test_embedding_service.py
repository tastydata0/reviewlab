...
from worker.models.embedding import Embedding768, Embedding1536
...
async def test_embedding_service_create_and_read_768(mock_session):
    ...
    with patch(
        "worker.services.embedding.embed_768", AsyncMock(return_value=[0.1] * 768)
    ):
...
async def test_embedding_service_create_and_read_1536(mock_session):
    ...
    with patch(
        "worker.services.embedding.embed_1536", AsyncMock(return_value=[0.2] * 1536)
    ):
